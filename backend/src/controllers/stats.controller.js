...
const getDashboardStats = asyncHandler(async (req, res) => {
    ...
    const currentWeekSLARisk = await Ticket.countDocuments({
        ...
        $or: [
            { slaBreach: true },
            { slaDeadline: { $lte: new Date(now.getTime() + 4 * 60 * 60 * 1000) } }
        ]
    });
    ...
    const previousWeekSLARisk = await Ticket.countDocuments({
        ...
        $or: [
            { slaBreach: true },
            { slaDeadline: { $lte: new Date(oneWeekAgo.getTime() + 4 * 60 * 60 * 1000) } }
        ]
    });
...
const getCriticalTickets = asyncHandler(async (req, res) => {
    ...
    const ticketsWithSLA = tickets.map(ticket => {
        ...
        if (ticket.slaBreach) {
            slaStatus = 'Breached';
        } else if (slaDeadline && slaDeadline < new Date(now.getTime() + 4 * 60 * 60 * 1000)) {
            slaStatus = 'At Risk';
        }
                        ...
                        $cond: [
                            {
                                $and: [
                                    { $eq: ['$slaBreach', false] },
                                    { $ne: ['$slaDeadline', null] },
                                    { $lt: ['$slaDeadline', { $add: [new Date(), 4 * 60 * 60 * 1000] }] }
                                ]
                            },
                            1,
                            0
