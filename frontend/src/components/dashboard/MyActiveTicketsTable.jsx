...
export default function MyActiveTicketsTable({ tickets = [], loading = false }) {
    const getSlaCssClass = (ticket) => {
        if (!ticket.slaDeadline) return 'text-muted-foreground';
        const now = new Date();
        const deadline = new Date(ticket.slaDeadline);
        const hoursUntilDeadline = (deadline - now) / (1000 * 60 * 60);

        if (hoursUntilDeadline < 0) return 'text-destructive font-bold';
        if (hoursUntilDeadline < 1) return 'text-orange-600 font-bold';
        if (hoursUntilDeadline < 4) return 'text-amber-600';
    ...
    if (tickets.length === 0) {
        return (
            <EmptyState
                ...
                description="You don't have any active tickets assigned right now."
            ...
            <Table>
                <TableHeader className="bg-muted/30">
                    <TableRow className="hover:bg-transparent border-none">
                        ...
                        <TableHead className="font-bold text-[10px] uppercase tracking-widest text-center">SLA Countdown</TableHead>
